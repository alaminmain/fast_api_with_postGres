...
const Dashboard = () => {
    ...
    // useEffect with dependency on user
    useEffect(() => {
        if (user) {
            fetchData();
        } else {
        ...
        <div className="space-y-6">
            ...
            {!user ? (
                <div className="bg-white rounded-xl shadow-sm p-10 border border-gray-100 text-center">
                    <h3 className="text-xl font-bold text-gray-800 mb-2">Welcome to Stock Manager</h3>
                    <p className="text-gray-500 mb-6">Track your portfolio and watchlist with ease. Please login to continue.</p>
                    <div className="space-x-4">
                        <Link to="/login" className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium">Login</Link>
                        <Link to="/register" className="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium">Register</Link>
                    </div>
                </div>
            ) : (
                <>
